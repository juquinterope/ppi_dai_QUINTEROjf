...
# Ejemplo de uso
nombre_municipio = "ANZÁ"  # Cambia esto por el nombre del municipio que deseas consultar
...
municipios_cercanos['temperatura'] = municipios_cercanos.apply(
                                        lambda x: obtener_datos_climaticos(x['Latitud'], x['Longitud'])['temperatura'],
                                        axis=1)
...
gdf = gpd.GeoDataFrame(
    municipios_cercanos, 
    geometry=gpd.points_from_xy(municipios_cercanos.Longitud, municipios_cercanos.Latitud)
    )

# Asignar el CRS inicial (WGS 84)
gdf = gdf.set_crs(epsg=4326)  # EPSG:4326 es para coordenadas geográficas (lat/lon)
